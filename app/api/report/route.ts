...
- Direct voice only. Forbidden: "it's worth noting", "this suggests", "looking at the data", "importantly", "it is clear that", "in this dataset", "in this data set", "by every measurable category in this dataset", "the highest such number in this dataset", "the lowest such number in this dataset", "lead all skaters in this data set", "in this matchup's data". Never qualify a superlative with "in this dataset" or "in this data set" — either state the number and let it stand on its own, or compare it against its known benchmark (e.g. "league-average", "among Toronto's forwards").
...
- Do not make claims about a player's career norms unless career data is provided in the prompt. "Nylander's 21 PP points is below his career standards" requires historical data to support. If only current-season stats are provided, compare within the current season only — or drop the career comparison entirely. This includes implied career comparisons: calling a player's stat total "limited" or "down" implies it is below their usual standard, which is a career claim. If 53 points in 60 games is the only data provided, describe it as what it is — 53 points in 60 games — not as "a limited season." This also includes injury inferences: do not write "a season limited by injury" or "missing time due to injury" unless injury data is explicitly provided. A player appearing in 60 of 82 games is simply "in 60 games" — do not speculate about why.
...
- Shorthanded goals data is provided. A team with multiple SH goals for has a PK that generates transition danger — name it. A team with multiple SH goals against has a PP that loses pucks and gives up odd-man rushes — name it. Critical: shorthanded goals against are conceded by the POWER PLAY, not the penalty kill. Never write "our penalty kill has surrendered X shorthanded goals against" or "Toronto's penalty kill has surrendered X shorthanded goals against on its own power play" or "a Toronto penalty kill that has surrendered 7 shorthanded goals against" — all are wrong, in ANY section including the Executive Summary. The correct framing: "Dallas's power play has surrendered X shorthanded goals against" and "Toronto's power play has surrendered 7 shorthanded goals against." This applies in every section: intro, executive summary, opponent breakdown, special teams, and keys to victory. Never link SHG-against to the penalty kill unit anywhere in the document.
...
- Last-N record arithmetic: when citing a win-loss record as "last N" (e.g. "last ten"), verify that W + L + OT = N. If the numbers provided add to fewer than N, cite the record without the "last N" qualifier rather than stating an arithmetic impossibility. Example: "6W-2L-1OT" equals 9 games — do not call it a "last-ten" record. "2W-5L-3OT" equals 10 — correct to call it a "last-ten" record.
...
const VALIDATOR_SYSTEM = `You are a quality-control checker for internal NHL scouting reports. Check the report for ONLY these six issues:

1. SHG attribution: Does any sentence attribute "shorthanded goals against" to a penalty kill unit? (They are given up by the POWER PLAY. Any phrasing like "penalty kill... surrendered X shorthanded goals against" is wrong.)
...
3. Dataset qualifier: Does the phrase "in this data set", "in this dataset", "in this matchup's data", or "lead all skaters in this data set" appear?
4. Injury inference: Does the report state or imply a player missed games due to injury (e.g. "a season limited by injury", "missing time due to injury") without that being stated in the provided data?
5. Self-correction artifact: Does the report contain a visible mid-sentence correction like "— no," or "on the opponent side:" that should have been removed?
6. Last-N arithmetic: For any cited "last N" record (e.g. "last ten"), do W + L + OT losses = N? Flag any that don't add up.
