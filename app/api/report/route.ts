...
- Direct voice only. Forbidden: "it's worth noting", "this suggests", "looking at the data", "importantly", "it is clear that", "in this dataset", "by every measurable category in this dataset", "the highest such number in this dataset", "the lowest such number in this dataset". Never qualify a superlative with "in this dataset" — either state the number and let it stand on its own, or compare it against its known benchmark.
...
- Shorthanded goals data is provided. A team with multiple SH goals for has a PK that generates transition danger — name it. A team with multiple SH goals against has a PP that loses pucks and gives up odd-man rushes — name it. Critical: shorthanded goals against are conceded by the POWER PLAY, not the penalty kill. Never write "our penalty kill has surrendered X shorthanded goals against" — that is wrong. The correct framing: "Dallas's power play has surrendered X shorthanded goals against, meaning the unit loses pucks and gives up transition." Shorthanded goals against belong in the Power Play section, not the Penalty Kill section.
