...
function scanBannedPhrases(text: string): string[] {
  ...
  check(
    ["true threat level", "below his true", "below her true", "below their true", "his true level", "her true level"],
    (p) => `Implied career comparison: "${p}" — no career data is provided; describe the player's current-season rate only, or compare against league averages`
  );

  // Face-off ranking superlatives — must verify win rate is highest among all named centers
  check(
    ["most reliable draw man", "most reliable center on draws", "most reliable face-off"],
    (p) => `Unsupported face-off superlative: "${p}" — verify this player has the highest face-off win rate among ALL named centers in the data before using this phrase`
  );
