...
- Face-off zone context for power play and penalty kill: when discussing how an opponent's POWER PLAY sets up in Dallas's defensive zone, cite the opponent center's OFFENSIVE-ZONE win rate (OZ%) — that is the draw they will take in the attacking zone. When discussing how Dallas's centers defend on the PENALTY KILL (draws in Dallas's defensive zone), cite the opponent center's OZ% as the threat, or Dallas's own DZ% as the response. Never cite an opponent center's DZ% as evidence of their power play threat — DZ draws happen in their own end, not while attacking. EXAMPLE OF THE ERROR: "Matthews wins 56.8% of defensive-zone draws — when he lines up for a PK draw against Dallas" is wrong. The correct framing: "Matthews wins 61.7% of offensive-zone face-offs — on Toronto's power play, he will dominate possession setup in Dallas's zone."
...
function scanBannedPhrases(text: string): string[] {
  ...
  check(
    ["woll's counterpart", "his counterpart faces", "the counterpart faces", "the opposing counterpart"],
    (p) => `Goalie circumlocution: "${p}" — name the goalie directly (e.g. "limit scoring chances against Oettinger") instead of using a roundabout reference`
  );
